...
[![Docs](https://img.shields.io/badge/Docs-CoinW.Net-1b7f50?style=for-the-badge)](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=CoinW.Net)
...
The [CoinW.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=CoinW.Net) is the main resource for installing, configuring, and using the library.

| Resource | Description |
|--|--|
| [Client guide](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=CoinW.Net) | Installation, REST and WebSocket clients, authentication, dependency injection, error handling, and advanced features |
| [Examples](https://cryptoexchange.jkorf.dev/docs/exchange-clients/examples?library=CoinW.Net) | Common REST and WebSocket operations |
| [API reference](https://cryptoexchange.jkorf.dev/docs/exchange-clients/reference?library=CoinW.Net) | Client interfaces, methods, and properties |
...
For more examples and explanations, continue with the [CoinW.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=CoinW.Net) or browse the [compilable repository examples](https://github.com/JKorf/CoinW.Net/tree/main/Examples).

## AI / LLM documentation

CoinW.Net includes AI-oriented documentation and examples for code generation tools:

|File|Purpose|
|--|--|
|[`AGENTS.md`](AGENTS.md)|Assistant skill with core CoinW.Net patterns, pitfalls, and examples|
...
This allows the same application code to work with different exchange libraries. The supported CoinW API surfaces expose their shared functionality through a `SharedClient` property. Because support differs between exchanges and API surfaces, call `Discover()` to inspect the available trading modes, environments, endpoints, and subscriptions at runtime.

### Supported shared interfaces

| API | Type | Supported interfaces |
|--|--|--|
| `SpotApi` | REST | `IAssetsRestClient`, `IBalanceRestClient`, `IDepositRestClient`, `IKlineRestClient`, `IOrderBookRestClient`, `IRecentTradeRestClient`, `ISpotOrderRestClient`, `ISpotSymbolRestClient`, `ISpotTickerRestClient`, `ITransferRestClient`, `IWithdrawalRestClient`, `IWithdrawRestClient` |
| `SpotApi` | WebSocket | `IBalanceSocketClient`, `IKlineSocketClient`, `IOrderBookSocketClient`, `ISpotOrderSocketClient`, `ITickerSocketClient`, `ITickersSocketClient`, `ITradeSocketClient` |
| `FuturesApi` | REST | `IBalanceRestClient`, `IFeeRestClient`, `IFuturesOrderRestClient`, `IFuturesSymbolRestClient`, `IFuturesTickerRestClient`, `IFuturesTpSlRestClient`, `IKlineRestClient`, `IOrderBookRestClient`, `IRecentTradeRestClient` |
| `FuturesApi` | WebSocket | `IBalanceSocketClient`, `IFuturesOrderSocketClient`, `IKlineSocketClient`, `IOrderBookSocketClient`, `IPositionSocketClient`, `ITickerSocketClient`, `ITradeSocketClient` |

### Discover supported functionality

```csharp
var sharedClient = new CoinWRestClient().SpotApi.SharedClient;
...
* Version 3.4.0 - 21 Aug 2026
    ...
    * Added TradfiTag to CoinWFuturesSymbol model
    * Unmapped incorrect Volume mapping for Shared GetFuturesTickersAsync and GetFuturesTickerAsync
...
* Version 3.2.0 - 21 Jul 2026
    ...
    * Updated PartitionsIds to array type on CoinWFuturesSymbol model

* Version 3.1.1 - 13 Jul 2026
...
* Version 3.0.0 - 29 Jun 2026
    ...
    * Added setter to CoinWExchange.RateLimiter to allow custom rate limit settings
    * Various small performance improvements
    * Fixed error message response parsing
